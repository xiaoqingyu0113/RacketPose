...
class SingleImageAlphaPose():
    def __init__(self, args, cfg):
        ...
        detector = YOLODetector(yolo_cfg, self.args)
        self.det_loader = DetectionLoader(detector, self.cfg, self.args)
...
def example():
    ...
    demo = SingleImageAlphaPose(settings, cfg)
    ...
    print(pose)
    cx,cy,cw,ch = pose['result'][0]['bbox']
    cx = int(cx);cy=int(cy);cw = int(cw); ch = int(ch)
    overlay = np.zeros_like(img, dtype=np.uint8)
    cv2.rectangle(overlay, (cx,cy), (cx+cw,cy+ch), (128,128,128), -1)
    result = cv2.addWeighted(overlay, 0.3, img, 1.0, 0)
    ...
    result = [pose]
    # demo.writeJson(result, outputpath, form=settings.format, for_eval=settings.eval)
    print(image.dtype)
